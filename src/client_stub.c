...
int rtables_put(struct rtables_t *rtables, char *key, struct data_t *value)
{
	...
	//Ja nao precisamos da mensagem de envio
	...
	//Ja nao precisamos da mensagem 2 ( de resposta )
	free(msg_2);
	return result;
}
...
int rtables_update(struct rtables_t *rtables, char *key, struct data_t *value)
{
	...
	int result = msg_2->content.result;
	free(msg_2);
	return result;
}
...
struct data_t *rtables_get(struct rtables_t *tables, char *key)
{
	...
	if (tables->print == 1)
		print_message(msg_2);
	struct data_t *data;
	data = data_dup(msg_2->content.data);
	free(msg_2);
	return data;
}
...
char **rtables_get_keys(struct rtables_t *rtables)
{
	...
	rtables_free_keys(msg_2->content.keys);
	free(msg_2);
	return arrkeys;
}
